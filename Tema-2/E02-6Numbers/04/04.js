...
function hasSumGreaterThan16(cardNumber) {
  ...
  const valid =
    numbers
      ...
      .reduce((acc, number) => acc + number) > 16;
...
function validateNumber(cardNumber) {
  ...
  const error = validationArray.find(
...
function validateCreditCard({ cardNumber, expirationDate }) {
  ...
  const error = valid
    ? ERROR.EMPTY
    : validationArray.find((validation) => validation.valid === false).error;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  form.addEventListener("submit", () => {
    ...
    const createADate = (date) => {
      ...
      return new Date(Number(`20${year}`), Number(month % 13));
    };

    const formatExpirationDate = createADate(expirationDate);
    const validatedCreditCard = validateCreditCard({
      cardNumber,
      expirationDate: formatExpirationDate,
    });

    alert(
      validatedCreditCard.valid
        ? `La tarjeta ${validatedCreditCard.number} es valida`
        : validatedCreditCard.error
    );
  });
});
